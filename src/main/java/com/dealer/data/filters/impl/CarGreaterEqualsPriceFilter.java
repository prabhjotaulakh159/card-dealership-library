...
public class CarGreaterEqualsPriceFilter extends VariousFilters implements ICarFilter {
    private int price;

    public CarGreaterEqualsPriceFilter(ListFilter filter, int price){
      super(filter);
      if(price<=0){
    ...
    public List<Car> filterCars(List<Car> cars){
      ...
      for(int i=0;i<cars.size();i++){
        if(!(super.greaterThanEqualsTo() && cars.get(i).getPrice() >= this.price)){
          cars.remove(i);
          i--;
        }
      }
